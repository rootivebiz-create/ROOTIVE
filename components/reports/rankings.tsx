  ...
  if (rows.length === 0) {
    return (
      <div className="px-4 pb-4">
        <Empty title="ドライバー別のデータはありません" description="この年に稼働行が登録されていません。" />
      </div>
    );
  }
  const total = {
...
export function ProjectRankingTable({ rows }: { rows: ProjectRankRow[] }) {
  if (rows.length === 0) {
    return (
      <div className="px-4 pb-4">
        <Empty title="案件別のデータはありません" description="この年に稼働行が登録されていません。" />
      </div>
    );
  }
  const total = {
...
export function ExpenseRankingTable({ rows, fixed, variable, total }: { rows: ExpenseRankRow[]; fixed: number; variable: number; total: number }) {
  if (rows.length === 0) {
    return (
      <div className="px-4 pb-4">
        <Empty title="経費は登録されていません" description="経費を登録すると、カテゴリ別の内訳と営業利益がここに反映されます。" />
      </div>
    );
  }
  return (
