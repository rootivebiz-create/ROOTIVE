...
const MAIN_PAGES: ({ href: string; label: string; keywords: string[] } & RoleVisibility)[] = [
  ...
  { href: "/dispatch?tab=offs", label: "休み希望", keywords: ["yasumi", "やすみ", "休み", "希望", "有給", "定休"] },
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const badges: Record<string, number> = {
    ...
    "/office": role === "owner" || role === "admin" || role === "clerk" ? badgeCounts.office : 0,
  };
  ...
  return (
    <AppShell
      ...
      fiscal={fiscalSettingsOf(company)}
    >
      {children}
    </AppShell>
  );
