...
- 財務の予算：年次レポートと同じ `ReportRange`（`?fy=` / `?y=`。既定は今日の期）で月を並べる（`toBudgetRowsForMonths`）。
  「前期実績 ＋ ◯%」は各月の 12 か月前（`previousMonths`）。保存は年ではなく「続いた 12 か月の範囲に収まるか」で確かめる（期は 2 つの暦年にまたがる）
- 借入の「今期の返済」は今期の最初の日〜最後の日（`periodDateRange`）で数える。税務の期限は日付のカレンダーなので暦年のまま、年にその年に決算を迎える期を添える
- 中期計画（0031）：計画の年は「期の決算の年」と読み替えた（列はそのまま。12 月決算なら暦年と同じ）。
  `fiscal_end_year(month, fiscal_month)`（`periodEndYearOf` と同じ式）で `v_plan_year_actual` の実績を期ごとに合計し、
  `spread_plan_year` は期の月へ配る（第1期は設立の月から。端数は決算月。`actual` は 12 か月前の同じ月の売上の構成比）
