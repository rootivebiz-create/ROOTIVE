...
| **定期アクセスの確認** | `CRON_SECRET` は自動生成されて Vercel に登録済みです。Vercel → プロジェクト `rootive-profit` → **Settings → Cron Jobs** に `/api/cron/keepalive`（毎日 6 時）が並び、直近の実行が **200** になっていることを確認します（`CRON_SECRET` が無いと 503 になり、Supabase の一時停止を防げません） |
