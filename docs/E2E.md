...
| `85-nav-portal.spec.ts` | ナビ（PC は 19 項目・入力／経営／管理／相談の見出し、スマホは下タブ 5 つ＋「メニュー」シートから経費へ）→ コマンドパレット（⌘K：「けいひ」で経費へ、「相曽」でドライバー候補、Esc で閉じる）→ 設定サブナビに取引先・経費カテゴリ → **ドライバーポータルの速報**（未締め月が「集計中」・税込 ¥436,307・「締め前のため変わることがあります」・年間サマリー）→ 会社設定で速報を止められる | `resetToSeed()` |
