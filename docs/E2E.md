...
| `25-office.spec.ts` | **事務**（管理者で `/office`：今日やることに承認待ち・休み希望・報告がまだの人 → 「確かめて承認」で 1 件を承認 → 休み希望に「承認」→ 今日の報告は 済み／まだ／連絡手段なし（代わりに入力）→ LINE 連携済みの 1 人に「まとめて催促」→ 催促済み・ボタンが消える（1 日 1 回）→ 2026年9月の締めの手順 10 件・手作業のチェックの付け外しと付けた人の名前 → 「最初に開く画面にする」で `/` が事務へ（スマホは下タブの先頭も事務）→ アカウントでホームに戻す → 閲覧者は `/office` を開けずナビにも出ない） | `resetToSeed()` ＋ 配車・休み・催促・チェックの削除、承認待ちと休み希望を SQL で用意。終わったら `resetToSeed()` |
