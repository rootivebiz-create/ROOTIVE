...
**最も簡単なのは GitHub Actions からの自動公開です**（ブラウザだけで完了・実績あり）。[docs/QUICKSTART.md](QUICKSTART.md) の「方法 A」を参照してください。
本書は、同じことを 1 つずつ手で行う場合の手順です。自動公開のあとに「どこに何が設定されたか」を確認する目的でも使えます。
...
> 実際の本番環境は、本番 URL `https://rootive-profit.vercel.app`（Vercel のエイリアス）、Supabase プロジェクト参照 ID `rmchixgmeqtszqsdvxrj`（東京）です。
...
2. リポジトリの [`supabase/seed/bootstrap_owner.sql`](../supabase/seed/bootstrap_owner.sql) を開き、**ファイル全体をコピー**して SQL Editor に貼り付けます（先頭のコメント行を含めてそのまま貼って構いません。psql 専用の `\set` 行はありません）。
3. 貼り付けた SQL の `do $$` の下、`declare` に続く 3 行の値を必要に応じて書き換えます。

   ```sql
   v_owner_email text := 'rootive.biz@gmail.com';        -- オーナーのメールアドレス
   v_company_name text := '株式会社ROOTIVE';              -- 会社名
   v_app_url text := 'https://rootive-profit.vercel.app'; -- 本番 URL（1 か所だけ）
   ```

   - `v_app_url` は既定で実際の本番 URL `https://rootive-profit.vercel.app` になっています。同じならそのままで構いません。違う URL にした場合は末尾に `/` を付けないでください。
   - 本番 URL がまだ決まっていない場合も、このままで構いません。招待リンクは `<本番URL>/invite/<英数字のトークン>` という形で、**大切なのは末尾のトークン部分**です。ドメイン部分は後から読み替えられます。
4. **「Run」** を押します。
5. 結果は 2 か所に出ます。
   - 結果欄の **「Messages」**（NOTICE）に、`会社を作成しました`／`オーナー招待を作成しました` に続いて **招待リンク**（`https://rootive-profit.vercel.app/invite/xxxxxxxx…`）が表示されます。これを**メモ帳に控えます**。最初のオーナーログイン用リンクです（有効期限 30 日、1 回のみ使用可）。
   - 結果欄の **「Results」** には、ファイル末尾の `select` による 1 行の表が出ます。**`invite_path` 列**（`/invite/xxxxxxxx…`）の前に本番 URL を付けたものが招待リンクです（例：`https://rootive-profit.vercel.app` + `/invite/xxxx…`）。Messages が見つからない場合はこちらを使ってください。
   - この SQL をもう一度実行すると、前の招待は取り消され**新しい招待リンク**が発行されます（古いリンクは使えなくなります）。期限切れになったときも同じ手順で再発行できます。
...
> **自動公開（QUICKSTART の方法 A／B）を使った場合も、この手順は手作業です。** 無料プランで Supabase 標準メールを使っている間は、テンプレートを API から変更できません（HTTP 400。スクリプトは警告を出して続行します）。手順 12 のカスタム SMTP を設定すると API からも変更できるようになります。招待リンク＋パスワードで運用する（メールを使わない）なら、この手順は後回しでも構いません。
...
- 本文の `{{ .SiteURL }}`・`{{ .TokenHash }}`・`{{ .RedirectTo }}` は Supabase が自動で差し込む部分です。書き換えないでください（`{{ .RedirectTo }}` はアプリが指定した「ログイン後に戻る画面」で、パスワード再設定なら「アカウント」画面、ドライバーなら自動でポータルへ移動します）。
...
5. **「Continue to Dashboard」** を押すと、**Domains** に本番 URL（`rootive-profit.vercel.app`）が表示されます。これを控えます。
   - Vercel はデプロイのたびに `rootive-profit-xxxx-….vercel.app` のような一意の URL も作りますが、使うのは常に **エイリアス**の `https://rootive-profit.vercel.app` です（常に最新の本番デプロイを指します）。
...
- 同じ `vercel.json` の `crons` により、毎日 6 時（日本時間）に `/api/cron/keepalive` が自動で呼ばれ、Supabase 無料プランの「7 日間アクセスが無いと一時停止」を防ぎます。Vercel はこの呼び出しに `CRON_SECRET` を自動で付け、アプリ側が照合します。**`CRON_SECRET` が未設定だと 503（無効）になる**ので必ず入れてください。実行結果は Project → **Settings → Cron Jobs** で確認できます（直近の実行が 200 なら正常）。
...
手順 8 で表示された本番 URL が、手順 3・4・8 で入れた予定値と**違う場合**（例 `rootive-profit-abc123.vercel.app` になった）は、次の 3 か所を直します。同じなら何もしなくて構いません（実際の本番は `https://rootive-profit.vercel.app` で、予定値と同じです）。
...
初期データ（ドライバー 10 名・案件 7 件・2026 年 9 月の稼働）を入れたい場合は、ログイン後に **設定 → データ** の「初期データ投入」カードにある **「サンプル初期データを投入（§8.6）」** を押すか（このカードはドライバーが 0 件のときだけ表示されます）、試作アプリの JSON を「取り込み・復元（オーナー）」から取り込みます（[docs/OPERATIONS.md](OPERATIONS.md) 参照）。
...
| 6 | 月締め | 設定 → 月締め → 対象月の「この月を締める」 | 状態が「締め済み」になり、稼働画面の編集ボタンが消える。同じ行の「バックアップ」ボタンで JSON が落とせる |
| 7 | 締め解除 | 同じ画面の「締めを解除」（オーナーのみ） | 再び編集できる |
| 8 | 閲覧者の編集不可 | 設定 → ユーザー管理 →「ユーザーを招待」で自分の別メールを「閲覧者」として招待し、そのリンクでログイン | 稼働・設定の編集ボタンが無い。設定 → ユーザー管理などの URL を直接開いてもダッシュボードに戻される |
| 9 | ドライバーポータル | ドライバー本人のメールを「ドライバー」ロール（対応ドライバーを選択）で招待し、そのリンクでログイン | 締め済み月の自分の明細だけが見える。他の人や会社利益は見えない |
| 10 | 試作 JSON の取り込み | 設定 → データ →「取り込み・復元（オーナー）」→ JSON ファイルを選ぶ → プレビュー →「取り込みを実行」 | プレビューの月別合計（売上・利益・支払）が試作アプリの数字と一致し、取り込み後のダッシュボードも一致する |
| 11 | 監査ログ | 設定 → 監査ログ | 上の操作が「誰が・いつ・何を」の形で並ぶ |
| 12 | 定期アクセス（Supabase 一時停止防止） | Vercel → プロジェクト `rootive-profit` → Settings → Cron Jobs | `/api/cron/keepalive` が一覧にあり、直近の実行結果が **200**。503 なら `CRON_SECRET` が未設定（手順 8） |
...
| 症状 | 原因 | 対処 |
|---|---|---|
| 招待リンクを開くと「招待リンクの有効期限が切れています」 | 招待は 7 日（最初のオーナー招待は 30 日）で失効 | オーナー：手順 3 の SQL をもう一度実行して新しいリンクを発行。他のユーザー：設定 → ユーザー管理 → 招待一覧の「リンクを再発行」（ログイン済みの人はユーザー一覧の「招待リンク」）で新しいリンクを発行して LINE で送る |
| 「この招待リンクは既に使用されています」 | 招待リンクは 1 回限り（同時に 2 回開いても 1 回しか通りません） | ログイン画面から「メールでログイン」か「パスワード」でログイン。メールが届かない・パスワード未設定なら、オーナーがユーザー一覧の「招待リンク」で新しいリンクを発行 |
...
| 自動公開のログに「メールテンプレートは API から設定できませんでした（HTTP 400）」 | 無料プランで Supabase 標準メールを使っている間はテンプレートを API から変更できない | 手順 5 のとおり手で貼り付ける。カスタム SMTP（手順 12）を設定すると API からも変更できる。公開処理そのものは成功している |
| Vercel のトークンが「無効」と言われる（スクリプト・CLI） | 貼り付け時に末尾へ改行や空白が混ざった／期限切れ | スクリプトは前後の空白・改行を自動で除去します。それでも失敗するなら Vercel → Account Settings → Tokens で再発行し、値の末尾に余計な行が無いことを確認 |
| Vercel → Settings → Cron Jobs の `/api/cron/keepalive` が 503 | `CRON_SECRET` が未設定 | 手順 8 の表のとおり 32 文字以上のランダム文字列を Environment Variables に追加 → Redeploy。自動公開なら再実行で自動生成される |
...
| 招待メールを送ったのに届かず、リンクも渡していない | Supabase 標準メールの制限 | 設定 → ユーザー管理 → 招待一覧の「リンクをコピー」（または「リンクを再発行」）で取り出し、LINE 等で直接送る |
