...
-- 生成元: 0001_schema.sql, 0002_auth_rls.sql, 0003_views.sql, 0004_rpc.sql, 0005_portal_seed.sql, 0006_storage_grants.sql, 0007_driver_rates.sql, 0008_tax_assets_payout.sql, 0009_expenses_invoices.sql, 0010_cashflow_project_pl.sql, 0011_ai_chat_integrations.sql, 0012_operations_compliance.sql, 0013_intake_hr.sql, 0014_finance_outputs.sql, 0015_backup_full.sql, 0016_daily_detect.sql, 0017_company_scoped_rpc.sql, 0018_labor_reconcile.sql, 0019_executive.sql, 0020_executive_guard.sql, 0021_perf.sql, 0022_notifications.sql, 0023_dispatch.sql
-- =============================================================================
...
-- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 0012_operations_compliance.sql >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
-- =============================================================================
...
-- >>>>>>>>>>>>>>>>>>>>>>>>>>>>>> 0017_company_scoped_rpc.sql >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
-- =============================================================================
-- 0017 cron から会社を指定して集計するときに、他社のデータが混ざらないようにする
--   cash_forecast と rate_diffs は RLS（呼び出し元の会社）に頼って絞り込んでいたため、
--   0016 で security definer になった detect_anomalies_core から呼ぶと RLS が効かず、
--   資金ショートの判定（11 番）が全社の合計になっていた。会社を明示して渡す版を用意して差し替える。
-- =============================================================================

-- ---------- 資金繰り（会社を明示。サービスロールと内部からのみ） ----------
create or replace function public.cash_forecast_for(p_company_id uuid, p_from date, p_to date)
returns table (
  event_date date,
  kind text,
  label text,
...
language sql stable security definer set search_path = public as $$
  ...
  select i.paid_on, 'invoice', cl.name, i.invoice_no || '（入金済み）', i.total, i.id, 'done', i.month
    from public.invoices i
    join public.clients cl on cl.id = i.client_id
   where i.company_id = p_company_id and i.status = 'paid' and i.paid_on is not null and i.paid_on between p_from and p_to
  union all
  -- ドライバーへの支払予定（税込）
  select public.month_day_date(s.month, coalesce(d.payout_month_offset, c.payout_month_offset), coalesce(d.payout_day, c.payout_day)),
         'payout',
  ...
  select coalesce(e.incurred_on, public.month_day_date(e.month, 0, 0)), 'expense', ec.name, e.label, -e.amount, e.id, 'done', e.month
   ...
   where e.company_id = p_company_id
     and coalesce(e.incurred_on, public.month_day_date(e.month, 0, 0)) between p_from and p_to
  union all
  -- これから計上される固定費（まだその月に計上されていないもの）
  select public.month_day_date(m.month, 0, coalesce(r.payment_day, 0)), 'expense', ec.name, r.label || '（予定）', -r.amount, r.id, 'planned', m.month
    from public.recurring_expenses r
  ...
  select lp.due_on, 'loan', l.name, l.lender || ' 第 ' || lp.seq || ' 回',
    ...
    from public.loan_payments lp
    join public.loans l on l.id = lp.loan_id
   where l.company_id = p_company_id and l.status <> 'planned' and lp.due_on between p_from and p_to
  order by 1, 2, 3;
$$;

-- ---------- 資金繰りの入口（スタッフ・自社のみ。画面と supabase-js はこちらを呼ぶ） ----------
create or replace function public.cash_forecast(p_from date, p_to date)
returns table (
  event_date date,
  kind text,
...
begin
  ...
  return query select * from public.cash_forecast_for(cid, p_from, p_to);
end $$;

-- ---------- マスタとの差分（会社を明示） ----------
create or replace function public.rate_diffs_for(p_company_id uuid, p_month date)
returns table (
  entry_id uuid, driver_id uuid, driver_name text, project_id uuid, project_item_id uuid, project_name text, item_name text, qty numeric,
  bill_rate numeric, pay_rate numeric, royalty_rate numeric, rounding_mode public.rounding_mode,
  master_bill_rate numeric, master_pay_rate numeric, master_royalty_rate numeric, master_rounding_mode public.rounding_mode
...
begin
  ...
  return query select * from public.rate_diffs_for(cid, p_month);
end $$;
...
revoke execute on function public.line_consume_code(text, text, uuid) from authenticated, anon, public;
revoke execute on function public.sync_work_entry_from_days(uuid, date, uuid, uuid) from authenticated, anon, public;
-- 会社を指定して集計できる版は、ログイン中のユーザーからは呼べないようにする
revoke execute on function public.detect_anomalies_core(uuid, date) from authenticated, anon, public;
...
-- =============================================================================
-- 0019 代表（Executive）
--   代表（owner）だけが見る・決めるための領域をつくる。
--   - 承認：管理者が申請し、代表が決裁する（高額の経費・単価の変更・新規案件・契約・借入・締め解除）
--   - 意思決定ログ：何を、なぜ決めたか。見直し日が来たら振り返る
--   - 会社の基本情報：登記・役員・株主・許認可・保険・顧問・個人保証（代表のみ）
--   - 中期計画：3 か年などの目標と、実績との対比
--   - ログインの記録：誰がいつ入ったか（代表のみ）
--   代表専用のテーブルは RLS で public.is_owner() を必須にする。
-- =============================================================================

-- ---------- 列挙型 ----------
do $$ begin
  create type public.approval_kind as enum
    ('expense','rate_change','project','contract','loan','month_reopen','payout','purchase','hire','other');
exception when duplicate_object then null; end $$;
...
do $$ begin
  create type public.decision_status as enum ('open','reviewed','dropped');
exception when duplicate_object then null; end $$;

do $$ begin
  create type public.login_event_kind as enum ('login','logout','invite');
exception when duplicate_object then null; end $$;
...
create table if not exists public.approvals (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.companies(id) on delete cascade,
  kind public.approval_kind not null default 'other',
  title text not null check (length(title) between 1 and 200),
  detail text not null default '',
  amount numeric(14,2),
  ref_table text not null default '',
...
create index if not exists approvals_company_idx on public.approvals (company_id, status, requested_at desc);
create index if not exists approvals_due_idx on public.approvals (company_id, due_on) where status = 'pending';
comment on table public.approvals is '代表の決裁が要る依頼。申請は管理者以上、決裁は代表のみ（RPC decide_approval）';
...
create index if not exists decisions_company_idx on public.decisions (company_id, decided_on desc);
create index if not exists decisions_review_idx on public.decisions (company_id, review_on) where status = 'open';
comment on table public.decisions is '意思決定の記録（代表のみ）。見直し日が来たら結果を書き、振り返る';
...
create table if not exists public.company_profile (
  ...
  corporate_number text not null default '' check (corporate_number = '' or corporate_number ~ '^[0-9]{13}$'),
  established_on date,
  capital numeric(14,2),
  registered_address text not null default '',
  representative_name text not null default '',
  business_purpose text not null default '',
  transport_office text not null default '',
  transport_number text not null default '',
  transport_notified_on date,
  labor_insurance_number text not null default '',
...
comment on table public.company_profile is '登記・許認可・社会保険などの会社の基本情報（代表のみ）';

create table if not exists public.officers (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.companies(id) on delete cascade,
  name text not null check (length(name) between 1 and 100),
  title text not null default '取締役',
  appointed_on date,
  term_end_on date,
  is_active boolean not null default true,
  memo text not null default '',
...
create index if not exists officers_company_idx on public.officers (company_id, sort_order);
comment on table public.officers is '役員（代表のみ）。任期満了が近づくと代表の画面で知らせる';
...
create index if not exists shareholders_company_idx on public.shareholders (company_id, sort_order);
comment on table public.shareholders is '株主名簿（代表のみ）';
...
create index if not exists insurance_company_idx on public.insurance_policies (company_id, expires_on);
comment on table public.insurance_policies is '会社の保険（代表のみ）。期限が近づくと知らせる';

create table if not exists public.advisors (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.companies(id) on delete cascade,
  kind text not null default '税理士',
  name text not null check (length(name) between 1 and 100),
...
create index if not exists advisors_company_idx on public.advisors (company_id, sort_order);
comment on table public.advisors is '顧問（税理士・社労士など。代表のみ）';

create table if not exists public.guarantees (
  id uuid primary key default gen_random_uuid(),
  company_id uuid not null references public.companies(id) on delete cascade,
  lender text not null default '',
  kind text not null default '個人保証',
  amount numeric(14,2) not null default 0,
...
create index if not exists guarantees_company_idx on public.guarantees (company_id, is_active);
comment on table public.guarantees is '個人保証・担保（代表のみ）。代表個人が負っているリスクを残す';
...
create table if not exists public.plans (
  ...
  to_year integer not null check (to_year between 2000 and 2100),
...
create index if not exists plans_company_idx on public.plans (company_id, is_active, from_year);
comment on table public.plans is '中期計画（代表のみ）。年ごとの目標は plan_years';
...
create table if not exists public.plan_years (
  ...
  bill_target numeric(14,2) not null default 0,
  profit_target numeric(14,2) not null default 0,
  driver_target integer not null default 0,
  memo text not null default '',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (plan_id, year)
);
create index if not exists plan_years_company_idx on public.plan_years (company_id, year);
comment on table public.plan_years is '中期計画の年ごとの目標（代表のみ）';
...
create table if not exists public.login_events (
  ...
  kind public.login_event_kind not null default 'login',
  ip text not null default '',
  user_agent text not null default '',
  at timestamptz not null default now()
);
create index if not exists login_events_company_idx on public.login_events (company_id, at desc);
comment on table public.login_events is 'ログイン・ログアウトの記録（代表のみ閲覧）。書き込みはサービスロール';

-- =============================================================================
-- 会社そのものの書類を置けるようにする（0012 は ドライバー か 車両 のどちらか必須だった）
--   登記簿謄本・運送事業の届出書・保険証券など、代表が保管しておきたい書類を kind='other' で置く
-- =============================================================================
...
declare t text;
begin
  foreach t in array array['approvals','decisions','company_profile','officers','shareholders',
                           'insurance_policies','advisors','guarantees','plans','plan_years']
  loop
    execute format('drop trigger if exists t00_set_updated_at on public.%I', t);
    execute format('create trigger t00_set_updated_at before update on public.%I for each row execute function public.set_updated_at()', t);
    execute format('drop trigger if exists t90_audit on public.%I', t);
...
-- 申請者・決裁者の名前を写す（閲覧者は他人の profiles を読めないため、行に持たせる）
create or replace function public.t_approval_names()
returns trigger language plpgsql security definer set search_path = public as $$
declare p record;
begin
  if new.requested_by is not null then
    select display_name, email into p from public.profiles where id = new.requested_by;
    if p is not null then
      new.requested_by_name := coalesce(nullif(p.display_name, ''), p.email, '');
    end if;
  end if;
  if new.decided_by is not null then
    select display_name, email into p from public.profiles where id = new.decided_by;
    if p is not null then
      new.decided_by_name := coalesce(nullif(p.display_name, ''), p.email, '');
    end if;
  else
    new.decided_by_name := '';
  end if;
...
drop trigger if exists t01_approval_names on public.approvals;
create trigger t01_approval_names before insert or update on public.approvals
  for each row execute function public.t_approval_names();

-- 決裁は代表だけ。決裁済みの申請は代表以外が触れない（画面・RPC に加えて DB でも拒否する）
  ...
  if tg_op = 'UPDATE' then
    if new.status is distinct from old.status then
      if new.status in ('approved', 'rejected') then
        if not public.is_owner() then
          raise exception '決裁できるのは代表だけです' using errcode = 'P0001', hint = 'FORBIDDEN';
        end if;
      end if;
...
drop trigger if exists t02_approval_guard on public.approvals;
create trigger t02_approval_guard before update on public.approvals
  for each row execute function public.t_approval_guard();

-- 意思決定の記録者の名前
create or replace function public.t_decision_name()
returns trigger language plpgsql security definer set search_path = public as $$
declare p record;
begin
  if new.created_by is not null then
    select display_name, email into p from public.profiles where id = new.created_by;
    if p is not null then
      new.created_by_name := coalesce(nullif(p.display_name, ''), p.email, '');
    end if;
  end if;
  return new;
end $$;
drop trigger if exists t01_decision_name on public.decisions;
create trigger t01_decision_name before insert or update on public.decisions
  for each row execute function public.t_decision_name();

-- 中期計画の年の company_id を計画から補完する
create or replace function public.t_plan_year_company()
returns trigger language plpgsql security definer set search_path = public as $$
declare c uuid;
begin
  select company_id into c from public.plans where id = new.plan_id;
  if c is null then
    raise exception '中期計画が見つかりません' using errcode = 'foreign_key_violation';
  end if;
  if new.company_id is null then
    new.company_id := c;
  elsif new.company_id <> c then
  ...
  if new.year < (select from_year from public.plans where id = new.plan_id)
     or new.year > (select to_year from public.plans where id = new.plan_id) then
    raise exception '計画の期間の外の年です' using errcode = 'check_violation';
  end if;
  return new;
end $$;
drop trigger if exists t01_plan_year_company on public.plan_years;
create trigger t01_plan_year_company before insert or update on public.plan_years
  for each row execute function public.t_plan_year_company();

-- =============================================================================
-- RLS
--   承認：スタッフ全員が見る（自分が出した申請の行方がわかる）。申請は admin 以上、決裁は代表のみ
--   それ以外の代表専用テーブル：public.is_owner() が無いと一行も見えない
-- =============================================================================
...
create policy approvals_update on public.approvals for update to authenticated
  using (company_id = public.current_company_id()
         and (public.is_owner() or (public.is_admin() and requested_by = auth.uid() and status = 'pending')))
  with check (company_id = public.current_company_id()
         and (public.is_owner() or (public.is_admin() and requested_by = auth.uid())));
...
declare t text;
begin
  foreach t in array array['decisions','company_profile','officers','shareholders',
                           'insurance_policies','advisors','guarantees','plans','plan_years','login_events']
  loop
    execute format('alter table public.%I enable row level security', t);
    execute format('drop policy if exists %I_owner on public.%I', t, t);
    execute format($p$create policy %I_owner on public.%I for all to authenticated
      using (company_id = public.current_company_id() and public.is_owner())
      with check (company_id = public.current_company_id() and public.is_owner())$p$, t, t);
  end loop;
end $$;
-- ログインの記録は書き込みもさせない（サービスロールが record_login_event で入れる）
drop policy if exists login_events_owner on public.login_events;
create policy login_events_owner on public.login_events for select to authenticated
  using (company_id = public.current_company_id() and public.is_owner());

-- =============================================================================
-- ビュー
-- =============================================================================
drop view if exists public.v_approval_list, public.v_plan_year_actual,
  public.v_executive_tasks, public.v_executive_summary cascade;

-- 承認の一覧（滞留日数と期限の遅れを添える）
...
comment on view public.v_approval_list is '代表への申請の一覧（滞留日数・期限切れの判定つき）';

-- 中期計画の年ごとの目標と、その年の実績
create view public.v_plan_year_actual
with (security_invoker = true) as
with actual as (
  select
    company_id,
    extract(year from month)::integer as year,
    coalesce(sum(bill), 0) as bill_actual,
    coalesce(sum(operating_profit), 0) as profit_actual,
    coalesce(max(active_driver_count), 0)::integer as driver_actual,
    count(*)::integer as month_count
  from public.v_month_pl
  group by company_id, extract(year from month)
)
select
  y.id,
  y.company_id,
...
comment on view public.v_plan_year_actual is '中期計画の年ごとの目標と実績（実績は v_month_pl の暦年合計）';

-- 代表がいま見るべきこと（代表専用のテーブルも混ざるため、alerts には出さずこのビューで出す）
...
comment on view public.v_executive_tasks is '代表がいま見るべきこと（決裁待ち・見直し日・保険の満了・役員の任期）';
...
-- =============================================================================
-- RPC：承認（申請は admin 以上、決裁は代表だけ）
-- =============================================================================
create or replace function public.request_approval(
  p_kind public.approval_kind, p_title text, p_detail text default '',
  p_amount numeric default null, p_ref_table text default '', p_ref_id text default '',
  p_href text default '', p_due_on date default null
) returns uuid language plpgsql security invoker set search_path = public as $$
declare cid uuid := public.current_company_id(); new_id uuid;
begin
  if not public.is_admin() then
    raise exception '権限がありません' using errcode = 'P0001', hint = 'FORBIDDEN';
  end if;
  if coalesce(trim(p_title), '') = '' then
    raise exception '件名を入れてください' using errcode = 'P0001';
  end if;
  insert into public.approvals (company_id, kind, title, detail, amount, ref_table, ref_id, href, due_on, requested_by)
  values (cid, coalesce(p_kind, 'other'), trim(p_title), coalesce(p_detail, ''), p_amount,
          coalesce(p_ref_table, ''), coalesce(p_ref_id, ''), coalesce(p_href, ''), p_due_on, auth.uid())
  returning id into new_id;
  return new_id;
end $$;

comment on function public.request_approval(public.approval_kind, text, text, numeric, text, text, text, date)
  is '代表に決裁をお願いする（admin 以上）';
...
declare cid uuid := public.current_company_id(); row_out public.approvals;
begin
  if not public.is_owner() then
    raise exception '決裁できるのは代表だけです' using errcode = 'P0001', hint = 'FORBIDDEN';
  end if;
...
comment on function public.decide_approval(uuid, boolean, text) is '申請を承認・却下する（代表のみ）';

create or replace function public.withdraw_approval(p_id uuid, p_note text default '')
returns public.approvals language plpgsql security invoker set search_path = public as $$
declare cid uuid := public.current_company_id(); row_out public.approvals;
begin
  if not public.is_admin() then
  ...
  update public.approvals
     set status = 'withdrawn', decision_note = coalesce(p_note, ''), updated_at = now()
   where id = p_id and company_id = cid and status = 'pending'
  returning * into row_out;
  if row_out.id is null then
    raise exception '取り下げできる申請が見つかりません' using errcode = 'P0001';
  end if;
  return row_out;
end $$;

comment on function public.withdraw_approval(uuid, text) is '申請を取り下げる（申請した本人か代表。DB トリガーでも確認する）';

-- =============================================================================
-- RPC：中期計画の年を作る（代表のみ）
-- =============================================================================
create or replace function public.ensure_plan_years(p_plan_id uuid)
returns integer language plpgsql security invoker set search_path = public as $$
declare cid uuid := public.current_company_id(); pl record; n integer := 0; y integer;
begin
  if not public.is_owner() then
    raise exception '権限がありません' using errcode = 'P0001', hint = 'FORBIDDEN';
  end if;
  select * into pl from public.plans where id = p_plan_id and company_id = cid;
  if pl.id is null then
    raise exception '中期計画が見つかりません' using errcode = 'P0001';
  end if;
  for y in pl.from_year .. pl.to_year loop
    insert into public.plan_years (company_id, plan_id, year)
    values (cid, pl.id, y)
...
comment on function public.ensure_plan_years(uuid) is '中期計画の期間ぶんの年を作る（すでにある年はそのまま。代表のみ）';

-- =============================================================================
-- RPC：ログインの記録（サービスロール専用。アプリのサーバー側から呼ぶ）
-- =============================================================================
create or replace function public.record_login_event(
  p_profile_id uuid, p_kind public.login_event_kind default 'login',
  p_ip text default '', p_user_agent text default ''
) returns uuid language plpgsql security definer set search_path = public as $$
...
begin
  ...
  insert into public.login_events (company_id, profile_id, email, display_name, role, kind, ip, user_agent)
  values (p.company_id, p.id, coalesce(p.email, ''), coalesce(nullif(p.display_name, ''), p.email, ''),
          p.role, coalesce(p_kind, 'login'), left(coalesce(p_ip, ''), 64), left(coalesce(p_user_agent, ''), 300))
  returning id into new_id;
  -- 古い記録は 1 年で捨てる（個人の行動履歴を必要以上に残さない）
  delete from public.login_events where company_id = p.company_id and at < now() - interval '1 year';
  return new_id;
end $$;

comment on function public.record_login_event(uuid, public.login_event_kind, text, text)
  is 'ログイン・ログアウトを記録する（サービスロール専用。1 年より古い記録は消える）';

-- =============================================================================
-- 0019 の続き：異常の検知に「決裁待ちの滞留」を足す（25）
--   代表専用のテーブル（保険・役員・意思決定）は alerts に出さない。
--   スタッフ全員が読める alerts に載せると代表だけの情報が漏れるため、v_executive_tasks で出す。
-- =============================================================================
create or replace function public.detect_anomalies_core(p_company_id uuid, p_month date)
returns jsonb language plpgsql security definer set search_path = public as $$
declare
...
-- =============================================================================
-- 0019 の続き：バックアップ（version 6）とデータ全削除に代表のテーブルを足す
--   代表専用のテーブルは export_backup（security invoker）の RLS により、
--   代表以外が書き出すと空の配列になる。import_backup はもともと代表のみが実行できる。
--   ログインの記録は監査ログと同じ扱いでバックアップに含めない。
-- =============================================================================
create or replace function public.export_backup()
returns jsonb language plpgsql security invoker set search_path = public as $$
declare cid uuid := public.current_company_id();
