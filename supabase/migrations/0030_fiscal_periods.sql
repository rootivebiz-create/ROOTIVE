...
-- 0030 期（事業年度）
--
--   画面で「第3期（2025年10月〜2026年9月）」のように期で見られるようにする。
--   期の区切りは決算月（companies.fiscal_month・0014）、番号は設立日から数える。
--
--   1. companies.established_on（設立日）
--      設立日はこれまで代表だけが読める会社の台帳（company_profile.established_on・0019）にしか無く、
--      スタッフの画面で期の番号を出せなかった。期を数えるためだけに companies にも持つ（会社名と同じく秘密ではない）。
--      - 台帳に入っていれば最初に写す
--      - 以後は 会社設定 と 会社の台帳 のどちらで変えても、もう一方に写す（同じ値なら何もしないので行き来しない）
--   2. 復元（import_backup）で設立日も戻す（書き出しは to_jsonb(companies) なので既に入っている）
-- =============================================================================

-- ---------- 1. 設立日 ----------
alter table public.companies add column if not exists established_on date;
comment on column public.companies.established_on is '設立日（0030）。期（第N期）を数える。会社の台帳（company_profile.established_on）と同じ値に保つ';

update public.companies c
   set established_on = cp.established_on
  from public.company_profile cp
 where cp.company_id = c.id and cp.established_on is not null and c.established_on is null;

-- 会社設定 → 会社の台帳
create or replace function public.sync_established_to_profile()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if new.established_on is distinct from old.established_on then
    update public.company_profile set established_on = new.established_on
     where company_id = new.id and established_on is distinct from new.established_on;
  end if;
  return new;
end $$;

drop trigger if exists t20_sync_established on public.companies;
create trigger t20_sync_established after update of established_on on public.companies
  for each row execute function public.sync_established_to_profile();

-- 会社の台帳 → 会社設定（台帳だけに入れても期が数えられるように）
create or replace function public.sync_established_to_company()
returns trigger language plpgsql security definer set search_path = public as $$
begin
  if new.established_on is not null and (tg_op = 'INSERT' or new.established_on is distinct from old.established_on) then
    update public.companies set established_on = new.established_on
     where id = new.company_id and established_on is distinct from new.established_on;
  end if;
  return new;
end $$;

drop trigger if exists t20_sync_established on public.company_profile;
create trigger t20_sync_established after insert or update of established_on on public.company_profile
  for each row execute function public.sync_established_to_company();

revoke all on function public.sync_established_to_profile() from public, anon, authenticated;
revoke all on function public.sync_established_to_company() from public, anon, authenticated;

-- ---------- 2. 復元で設立日も戻す（0028 の import_backup に 1 行足しただけ） ----------
...
begin
  ...
  if c is not null and jsonb_typeof(c) = 'object' then
    update public.companies set
      ...
      -- 0030 で追加：設立日（期を数える）
      established_on = coalesce(nullif(c->>'established_on', '')::date, established_on),
