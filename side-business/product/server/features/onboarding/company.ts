...
/** 資本金・従業員の数を聞く理由（画面に 1 行で出す）。取適法は画面で最初に出すところで正式な名前も添える */
/**
 * 「ドライバーが持つ」を選んだときの注意。取適法の対象かどうかで区切らない
 * （フリーランス法でも、2026年1月1日以降の発注は合意があっても減額として扱われうる。見張り番・設定の文と同じ向き）
 */
export const FEE_DEDUCT_NOTE =
  "振込手数料をドライバーの負担にして振込額から差し引くと、報酬の減額にあたるおそれがあります（2026年1月1日以降に発注する取引は、合意があっても）。取引条件と、会社の負担にする設定の確認をおすすめします";

export const SIZE_REASON = "取適法（中小受託取引適正化法）の対象かの目安に使います";
/** 同じ画面で正式な名前をもう出したあと（振込手数料の注意のあと）の言い方 */
export const SIZE_REASON_SHORT = "取適法の対象かの目安に使います";
