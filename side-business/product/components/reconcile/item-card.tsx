...
import { amountText, formulaText, KIND_HELP, KIND_LABEL, qtyUnitText, STATUS_LABEL, STATUS_TONE } from "~/server/features/reconcile/labels";
  ...
  return (
    <Card>
      ...
      <p className="mt-2 text-xs text-muted-foreground">
        ...
        {item.kind === "extra" && !item.confirmedExtra && " 下の「お支払通知の行」で、案件・追加の料金・対象外のどれかに決められます。"}
      </p>
      {canEdit && item.id ? (
        <ItemStatusForm itemId={item.id} status={item.status} note={item.note} />
      ) : (
