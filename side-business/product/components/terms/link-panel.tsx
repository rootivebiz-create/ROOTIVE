...
import { markTermsSentAction, recreateTermsLinkAction } from "~/app/(app)/terms/actions";
import type { TermsChannel } from "~/server/features/terms/schema";

type Note = { tone: "ok" | "error" | "info"; text: string } | null;

/**
 * 明示書をドライバーへ送る：リンクのコピー・LINE・SMS・メール（押すと「送った」記録がつく）と、リンクの作り直し。
 * リンクの値は会社の画面でだけ作る（見るだけの人には出さない）。送れるのは最新の版だけ。
 */
export function TermsLinkPanel({
  recordId,
  url,
  message,
  links,
  ...
  function record(channel: TermsChannel, doneText: string) {
    startTransition(async () => {
      const r = await markTermsSentAction(recordId, channel);
    ...
    <div className="space-y-3">
      <label className="block">
        ...
        <input
          ...
          aria-describedby="terms-link-expiry"
        />
        <span id="terms-link-expiry" className="mt-1 block text-xs text-muted-foreground">
      ...
      <RecreateLink recordId={recordId} />
    </div>
  );
}

function RecreateLink({ recordId }: { recordId: string }) {
  const [state, action, pending] = useActionState(recreateTermsLinkAction, undefined);
  ...
  return (
    <div className="border-t border-border pt-3">
      {!open ? (
        <Button variant="ghost" onClick={() => setOpened({ at: state })}>
          リンクを作り直す
        </Button>
      ) : (
        <form action={action} className="rounded-lg border border-warning/40 bg-warning/10 p-3 text-sm">
          ...
          <p className="mt-1">
            間違った相手に送ったとき・リンクが人に知られたときに使います。送った記録は外れます（「受け取りました」の記録は残ります）。作り直したら、新しいリンクを送り直してください。
          </p>
