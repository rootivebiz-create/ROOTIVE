...
/** 月末の流れ（6 つ）。明細 → 締め → 振込の順（ホームの段・振込データと締めの画面の案内と同じ） */
export const MONTH_END_STEPS: HelpStep[] = [
  ...
  {
    title: "振込データを作る",
    body: "締めたあとで、明細の振込額から、銀行の「総合振込」にそのまま読み込めるファイル（全銀の形式）を作ります（締めてから作ると、あとで金額が変わる心配がありません）。銀行で振り込んだら、「実際に振り込んだ日」を入れておきます（約束した支払日と比べられます）。",
...
export const FAQS: Faq[] = [
  ...
  },
  {
    ...
    q: "振り込んだあとに間違いが分かったら",
    a: [
      "締めを外して直し、明細を作り直します。振り込んだ日を記録した振込データがあると、振り込んだ額と新しい明細の額が合わなくなります。同じ人に二重に振り込まないよう、その人は「まだ振込データに入っていない人だけ」の振込データには入りません。",
      "振込データの画面の「振り込んだ額と明細の額の差」に、人ごとに振り込んだ額・今の明細・差が出ます。人ごとに、精算の仕方を選んで記録してください。",
      "「翌月（〇月分）の明細で精算する」を選ぶと、翌月の「稼働と調整」に調整が 1 行入り、振り込んだ額とどの版の明細との差かが根拠に残ります。翌月の明細と振込データで、ほかの額といっしょに払えます。",
      "差額だけ別に振り込んだときや、払いすぎの分を返してもらったときは「別の方法で精算したと記録する」で、その日とメモを残します。",
      "払いすぎの分を翌月の明細から差し引くときは、先にご本人と話して、書面で合意した記録を残してください。どちらの仕方でも、ドライバーに直したことと精算の仕方を伝えてください。",
      "翌月の精算の行を会計ソフトへ書き出すときは、同じ費用を 2 回計上しないよう、扱いを顧問の税理士さんと確かめてください。",
    ],
    links: [
  ...
  },
  {
    ...
    q: "前の月・前の版の明細を探したい",
    a: [
      "「支払明細」の「過去の明細を探す」（明細の検索）で、期間・振込額の範囲・ドライバーを選んで探せます。作り直した前の版も開けます。",
      "見つかった明細の一覧（索引）は CSV で出せます。税理士さんや調査で「この期間の明細を」と言われたときに使ってください。",
    ],
    links: [{ href: "/records", label: "明細の検索", need: "viewer" }],
  },
  {
    id: "accounting",
    q: "会計ソフトに取り込みたい",
