    ...
    <div className="space-y-4">
      ...
      {canEdit && !hasGeneral && (
        <div className="rounded-card border border-dashed border-border p-3">
          ...
          <p className="text-xs text-muted-foreground">明細を直したときのお知らせなどに。ドライバーは同じリンクで読めます。</p>
    ...
    <section id={threadAnchor(t.lineKey)} className="scroll-mt-4 rounded-card border border-border bg-card p-3" aria-label={`${t.label}についてのやりとり`}>
      ...
      {canEdit && (
        <div className="mt-3 space-y-2">
          <ReplyForm statementId={statementId} lineKey={t.lineKey} placeholder="返事を書く（ドライバーは同じリンクで読めます）" />
