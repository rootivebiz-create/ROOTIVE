...
import { buildPayout, type Deduction, type Payee, type PayoutLine, type PayoutResult } from "@/lib/engine/statement";
...
function payoutExample(lines: PayoutLine[], deductions?: Deduction[]): PayoutResult {
  return buildPayout({
    payee: PAYEE,
    lines,
    deductions,
...
function buildModels(): Model[] {
  ...
  const koma = payoutExample([
    { label: "個別指導", model: "unit", input: { qty: 20, rate: 2_000, unitLabel: "コマ" }, withholding: "ko1" },
    { label: "面談手当", model: "unit", input: { qty: 2, rate: 500, unitLabel: "回" }, withholding: "ko1" },
  ]);
    ...
    [
      {
        label: "月謝の歩合",
        model: "commission",
        input: { categories: [{ label: "担当する生徒の月謝", sales: tuitionSales, rate: tuition.rate }] },
        withholding: "ko1",
      },
    ],
    [{ label: "教室の使用料", amount: roomFee, agreedInWriting: true, kind: "fee" }],
  );

  // レッスン ＋ 参加人数の加算
  const bonus: ThresholdInput = { counts: [9, 10, 8, 12, 7, 9, 11, 10], base: 8, unitPrice: 200, countLabel: "人" };
  const lessons = Array.isArray(bonus.counts) ? bonus.counts.length : 1;
  const lesson = payoutExample([
    { label: "レッスン", model: "unit", input: { qty: lessons, rate: 3_500, unitLabel: "本" }, withholding: "ko1" },
    { label: "人数の加算", model: "threshold", input: bonus, withholding: "ko1" },
  ]);
  ...
  const tutor = payoutExample([
    ...
    { label: "交通費", model: "fixed", input: { amount: visit.fare, text: `${visit.times}回分` }, withholding: "ko1" },
  ]);

  // 作問・教材の原稿
  const writing = payoutExample([
    { label: "テストの作問", model: "unit", input: { qty: 3, rate: 5_000, unitLabel: "本" }, withholding: "ko1" },
  ]);

  // 月額（担当クラス）＋ 追加のコマ、教室の運営の委託
  const monthly = payoutExample([
    { label: "担当クラス", model: "fixed", input: { amount: 60_000, text: "月額" }, withholding: "ko1" },
    { label: "追加の授業", model: "unit", input: { qty: 2, rate: 3_000, unitLabel: "コマ" }, withholding: "ko1" },
    {
      label: "教室の運営（受付・事務）",
      model: "fixed",
  ...
  return [
    {
      ...
      how: "授業1コマの単価 × コマ数に、面談・テスト監督・研修などの手当を足す。交通費を報酬と一緒に渡すことも多い",
    ...
    {
      name: "作問・教材の原稿",
      how: "1本・1問・1ページいくら。原稿の報酬として、授業と同じ1号。同じ月に授業の報酬と一緒に払うなら合計してから源泉を計算する",
      where: "テストの作問・教材やプリントの作成など",
      example: [`例：${writing.lines.map((l) => `${l.label} ${l.detail}`).join("、")}`, `→ 源泉：${formulas(writing)}`],
    },
    {
      ...
      {MODELS.map((m) => (
        <li key={m.name}>
          <Card className="h-full">
            ...
            {m.note && <p className="mt-2 text-xs leading-relaxed text-muted-foreground">{m.note}</p>}
            {m.caution && (
