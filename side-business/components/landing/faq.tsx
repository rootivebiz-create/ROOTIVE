...
export function landingFaq(isOperator: boolean = SITE.makerIsOperator): FaqItem[] {
  ...
  return [
    ...
    },
    {
      ...
      a: "データは御社のサーバーにだけ置きます。ドライバーの明細のリンクは検索に出ず、口座は下3桁だけを出します。当方が作業で本番の個人情報をAIに入れることはしません。製品のAIの機能は既定では使わず、御社のオーナーが同意したときだけ、取り込むファイルの列の見出しとはじめの数行を送ります（口座や振込データは送りません）。契約には個人情報の取り扱いの取り決めを付けます。お試しは、ドライバーの名前を番号に置きかえ、口座・住所の列を消したExcelで行えます。",
    },
