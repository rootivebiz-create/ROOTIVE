 ...
 *   400 入力の誤り（欄ごとの日本語） / 413 大きすぎる / 429 続けて送られた / 503 送り先が未設定 / 502 すべての送り先に届かなかった
...
/** ログに出す誤り（名前と文言だけ。応答の本文や URL は出さない） */
...
export async function POST(req: Request) {
  ...
  if (declared > MAX_BODY_CHARS * 4) return json(413, { error: CONTACT_TEXT.tooLarge });
  ...
  if (raw.length > MAX_BODY_CHARS) return json(413, { error: CONTACT_TEXT.tooLarge });
  ...
  results.forEach((r, i) => {
    if (r.status === "rejected") console.error(`[contact] ${deliveries[i].label} delivery failed: ${describeError(r.reason)}`);
  });
