    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div className="mt-6 grid gap-4">
        <Material
          ...
          qr={`相談・無料診断（${displayUrl("/contact")}）とデモ（${displayUrl("/demo")}）。utm_source=proposal 付き`}
        >
        ...
        <Material
          ...
          qr="きっかけごとに、計算ツール・明示書のツール・相談のページ。utm_source=fax 付き"
        >
            ...
            {FAX_HOOK_IDS.map((id) => {
              ...
              return (
                <li key={id} className="rounded-lg border border-border p-3">
                  <Link href={id === "70" ? "/kit/fax" : `/kit/fax?hook=${id}`} className="inline-flex min-h-11 items-center font-bold">
                    {h.name}
                  </Link>
                  <p className="text-sm leading-relaxed">見出し：{h.headline}</p>
                  <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
                    送るとよい時期：{h.when}／QR：{displayUrl(h.qrPath)}
                  </p>
                </li>
              );
            })}
        ...
        <Material
          ...
          qr={`トップページ（${displayUrl("/")}）。utm_source=flyer 付き`}
        >
