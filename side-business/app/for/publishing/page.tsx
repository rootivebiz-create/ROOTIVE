...
const PAY_MODELS = [
  ...
  { name: "撮影料", how: "カット数 × 1カットの単価、または日当 × 撮影日数（機材費を足すことも）", where: "取材・物撮り・イベントの撮影" },
  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <ul className="mt-4 space-y-1 border-l-4 border-accent pl-3">
        <li>
          原稿料・撮影料・イラスト料・デザイン料・印税は、払うときに源泉徴収をします。税率は{BASIC}、同じ人への1回の支払で
          {STEP}を超える部分だけ{UPPER}で、1円未満は切り捨てです。
        </li>
        <li>
          {STEP_NOW && STEP_PREV
            ? `${jpDate(STEP_NOW.from)}から、インボイス登録をしていない（免税の）方への支払で控除できる割合は、${pct(STEP_PREV.rate)}から${pct(STEP_NOW.rate)}になります。`
      ...
      <div className="prose-ja mt-4">
        <h2>源泉徴収</h2>
        <p>
          所得税法204条1項1号は、原稿・写真・挿絵・デザインの報酬や講演料、著作権の使用料（印税）などを、払うときに源泉徴収が必要な報酬として挙げています。個人のライター・カメラマン・イラストレーター・著者に払うときは、払う側が所得税と復興特別所得税を差し引いて、国に納めます。
        </p>
      </div>
      <Card className="mt-2">
      ...
      <Card className="mt-2">
        ...
        <p className="mt-2 text-sm leading-relaxed">
          2025年6月17日、公正取引委員会は小学館と光文社に、フリーランス法にもとづく勧告をしました。原稿の執筆や写真の撮影などを頼むときに取引条件を示さなかったこと、期日までに報酬を払わなかったことが理由です。雑誌などの刊行日を基準に払っていて、受け取ってから支払まで80日以上あいた例もあったと報じられています。
        </p>
      </Card>
      <div className="prose-ja">
        <p>
