...
const TITLE = "業務委託講師・インストラクターの報酬と源泉徴収（コマ給・月謝歩合の無料計算ツール）";
...
const STEP_INDEX = TRANSITIONAL_STEPS.findIndex(
  (s) => SCHOOL.serviceDate >= s.from && (s.to === null || SCHOOL.serviceDate <= s.to),
);
...
const MONTHLY_WITHHOLDING = calcWithholding("ko1", MONTHLY, { date: SCHOOL.serviceDate });

/** 取引条件明示書の「報酬の額」に書ける算定方法（最初の見本の行から作る） */
const FIRST_SAMPLE = SCHOOL.samples[0] ?? null;
const DISCLOSURE_TEXT = FIRST_SAMPLE
  ? FIRST_SAMPLE.input.lines.map((l) => `${l.label} ${calcModel(l).formulaText}`).join("、")
  : null;
...
const SOURCES = [
  ...
  {
    label: "弁護士による裁判例の解説（業務委託の塾講師の労働者性。東京地裁 令和5年9月26日判決。民間の解説）",
...
const ADJUSTMENTS = [
  ...
  "振込手数料：支払う側の負担が基本です。合意していても、報酬から一方的に差し引く運用は問題とされうるため、見直しをおすすめします",
];
...
export default function SchoolPage() {
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <p className="text-sm font-bold text-muted-foreground">業種別のまとめ：{SCHOOL.label}</p>
      ...
      <ul className="mt-4 space-y-1 border-l-4 border-accent pl-3">
        <li>
          授業・レッスンの報酬（教授料）は、個人の講師に払うときに源泉徴収をします。税率は{BASIC}、同じ人への1回の支払で{STEP}
        ...
        <li>
          ...
          フリーランス法では、頼んだらすぐに条件を示し、従業員がいる教室は受け取った日から60日以内に払います。
        </li>
      </ul>
      ...
      <p className="mt-2 text-xs text-muted-foreground">{AS_OF}時点の制度にもとづきます。例の教室・人はすべて架空です。</p>
      ...
      <div className="prose-ja mt-4">
        ...
        <p>
          ...
          です。学習塾の授業も、音楽・語学・ヨガ・ダンスなどのレッスンも同じです。テストの作問や教材の原稿は「原稿の報酬」で、同じ1号です。区分は講師ごとではなく、
            ...
            {SCHOOL.withholdingHints.map((h) => (
              <tr key={h.item}>
                ...
                <td className="border border-border px-2 py-2">
                  {h.category === "none" ? "しない（対象に挙げられた報酬にあたらない場合）" : `する（${WITHHOLDING_CATEGORIES[h.category].short}）`}
      ...
      <div className="prose-ja">
        ...
        <ul>
          <li>
            ...
            です。年の合計にかけるものではありません。たとえば毎月{en(MONTHLY)}ずつ{MONTHLY_TIMES}回払う講師（年
            {en(MONTHLY * MONTHLY_TIMES)}）でも、源泉は毎回 {MONTHLY_WITHHOLDING.formulaText} です。
          </li>
          <li>1円未満は切り捨てます（四捨五入しません）。</li>
          <li>1回の支払の中で、区分が同じ行（授業・手当・作問など）を合計してから計算します。</li>
          <li>
            請求書等で消費税がはっきり分けて書いてあれば、税抜の額にかけてかまいません。インボイス（適格請求書）でない請求書でも同じです。分けて書いていなければ、原則どおり税込の額にかけます。
            {taxBase && (
              <>
                上の例の{taxBase.name}なら、分けて書いてあるときは元 <Money value={taxBase.separated.base} />・源泉{" "}
                <Money value={taxBase.separated.withholding} />、分けて書いていないときは元 <Money value={taxBase.notSeparated.base} />
                ・源泉 <Money value={taxBase.notSeparated.withholding} /> です。
              </>
            )}
          </li>
          <li>
            報酬と一緒に講師へ渡す交通費は、原則として源泉の元に入ります（所得税基本通達204-4）。教室が交通機関などへ直接払ったものは入りません。
          </li>
          <li>
            教室の使用料などを相殺するときも、源泉は<strong>相殺の前の報酬の額</strong>にかけます（相殺で元は減りません）。
            {offset && (
              <>
                上の例の{offset.name}なら、元 <Money value={offset.base} /> に対して源泉 <Money value={offset.withholding} />
                、そのあとで{offset.deductionLabel} <Money value={offset.deduction} /> を差し引いて、振込は <Money value={offset.payout} />{" "}
                です。
              </>
            )}
          </li>
        ...
        <ul>
          ...
          {REPORT_OVER !== null && (
            <li>同じ講師への1年の支払が{en(REPORT_OVER)}を超えたら、翌年1月31日までに支払調書を出します。</li>
          )}
          {EXPECTED && (
            <li>
      ...
      <div className="prose-ja">
        <ul>
          ...
          <li>
            負担が出るのは、消費税を<strong>原則課税</strong>で計算している教室だけです。簡易課税や免税事業者の教室なら、講師が登録しているかどうかで納める消費税は変わりません
            {burden ? (
              <>
                （上の例と同じ支払でも、簡易課税の教室なら <Money value={burden.simplified} />）
              </>
            ) : null}
            。
          </li>
          {burden && (
            <li>
              上の例の{burden.first.name}なら、教室が控除できずに負担する消費税は
        ...
        <h2>フリーランス法</h2>
        <p>フリーランス法（2024年11月1日施行）は、従業員を使っていない個人の講師・インストラクターに仕事を頼む教室に関わります。</p>
        <ul>
          <li>
            <strong>取引条件の明示（3条）</strong>
            ：講師に仕事を頼んだら直ちに、業務の内容・報酬の額・支払期日などを、書面かメールなどで示します。報酬の額は、具体的な金額を決められない事情があれば、
            <strong>算定方法（1コマあたりの単価 × コマ数、月謝 × 率 など）で示してもかまいません</strong>
          ...
          <li>
            ...
            {late && (
              <>
                上の例の{late.name}は、{jpDate(late.receivedOn)}締めの分を{jpDate(late.payOn)}に払う条件で、60日の期限（
          ...
          <li>
            ...
            ：1か月以上の業務委託では、受領拒否・報酬の減額・買いたたき・購入や利用の強制・不当な経済上の利益の提供の要請・不当な内容の変更ややり直しなどが禁止されています。決めた報酬をあとから一方的に下げる、合意の無い差し引きをする、教材や物販を買わせる、授業の準備・研修・代講を無償で求める、といった運用は、これらにあたるおそれがあります。振込手数料は、支払う側の負担を基本にしてください。
          </li>
          <li>
          ...
          <li>
            公正取引委員会は2025年3月28日、フィットネスクラブ業を含む4業種（リラクゼーション業・フィットネスクラブ業・ゲームソフトウェア業・アニメーション制作業）を集中的に調べ、45名の事業者にフリーランス法にもとづく指導をしました。
          </li>
        </ul>
        {FIRST_SAMPLE && DISCLOSURE_TEXT && (
          <>
            <p>たとえば上の{FIRST_SAMPLE.input.payee.name}の条件なら、明示書の「報酬の額」は算定方法として次のように書けます。</p>
      ...
      <aside aria-labelledby="cautions-title" className="mt-10 rounded-card border-2 border-warning bg-card p-4">
        ...
        <ul className="mt-2 list-disc space-y-1 pl-5 text-sm leading-relaxed">
          <li>このページと計算の道具は、講師・インストラクターが労働者（雇用）にあたるか、偽装請負にあたるかを判定しません。</li>
          <li>
            時間割や教室の指定、指導方法やテキストの細かい指定、授業以外の業務（受付・報告書・チラシ配りなど）の指示、代わりの人を立てられないこと、勤怠と結びついた固定の月額などは、労働者性のリスクがあるとされる要素です。契約の名前ではなく実態で判断されます。業務委託の塾講師について、労働組合法上の労働者にあたるとした裁判例もあります（東京地裁 令和5年9月26日判決）。判断は弁護士・社会保険労務士などの専門家に確かめてください。
          </li>
          <li>実態が雇用と判断されると、給与としての源泉徴収や、消費税の仕入税額控除の扱いなども変わることがあります。</li>
          <li>
