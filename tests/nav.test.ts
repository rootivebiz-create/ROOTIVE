...
describe("ナビの定義", () => {
  it("PC のサイドナビは 19 項目（入力・経営・管理・相談のまとまり ＋ ホームと設定）", () => {
    expect(MAIN_NAV).toHaveLength(19);
    expect(MAIN_NAV.map((i) => i.href)).toEqual([
      "/dashboard", "/entries", "/daily", "/intake", "/payouts", "/invoices", "/expenses", "/bank",
      "/cashflow", "/projects", "/finance", "/reports", "/alerts", "/fleet", "/hr", "/exports", "/ai", "/chat", "/settings",
    ]);
    expect(MAIN_NAV.map((i) => i.label)).toEqual([
      "ホーム", "稼働", "日報・点呼", "取り込み", "支払", "請求", "経費", "入金",
      "資金繰り", "案件", "財務", "レポート", "気になること", "車両と書類", "採用と契約", "出力", "AI 相談", "チャット", "設定",
    ]);
  });
